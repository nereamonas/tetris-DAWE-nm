...
// TU CÓDIGO AQUÍ: Borra del grid y de pantalla todos los bloques de la fila y
	for (var x = 0; x < this.width; x++) { //Recorreremos todas las x
		item = x + "," + y;  //Sacamos el item
		removeBloquesActuales(this.grid[item]);//Eliminamos tambien de la lista de bloques pintados actualmente en el canvas
...
Tetris.prototype.init = function(){
	...
	console.log("NEXT SHEP");
	console.log(this.next_shape.constructor.name);
socket.emit('nextCurrent-shape',{jugador:"J1",nextShape: this.next_shape.constructor.name, currentShape:this.current_shape.constructor.name});
	// TU CÓDIGO AQUÍ:
	...
	pushBloquesActuales(this.current_shape); //Añadimos la nueva pieza a la lista de piezas dibujadas actualmente
...
Tetris.prototype.key_pressed = function(e) {
	...
	if(!gameOver) {
		if (!tetris_paused) {  //Si el tetris no está pausado
			if (key == 39) {  //Derecha
				socket.emit('move',{jugador:"J1",direction: "Right"});
				this.do_move("Right");
			} else if (key == 37) {  //Izquierda
				
				socket.emit('move',{jugador:"J1",direction: "Left"});
				this.do_move("Left");
			} else if (key == 40) {  //Abajo
				
				socket.emit('move',{jugador:"J1",direction: "Down"});
				this.do_move("Down");
			} else if (key == 32) {  //Barra espaciadora
				
				socket.emit('move',{jugador:"J1",direction: "Space"});
				tetris.do_move("Space");
			} else if (key == 38) {  //Arriba
				loadAudio("audio/sonido_rotar.mp3").then(audio => audio.play());
				this.do_rotate();
				socket.emit('rotate',{jugador:"J1"});
			} else if (key == 80) {  //P  - pausar juego
				console.log("Juego pausado");
				this.board.juego_pausado();

				socket.emit('pausarJuego',{jugador:"J1"});
				tetris_paused = true;
			...
			if (key == 80) {  //P  - pausar juego
				...
				socket.emit('reanudarJuego',{jugador:"J1"});
...
Tetris.prototype.do_move = function(direction) {
	...
	if (piezaFinal){
		...
		socket.emit('nextShape',{jugador:"J1",nextShape: this.next_shape.constructor.name});
...
var bloquesActuales = new Array();  //Array donde guardaremos todos los bloques que están actualmente pintados en el tablero

function pushBloquesActuales(newshape){ //Le pasamos una pieza y guardaremos cada bloque de la pieza en el array bloquesActuales
	newshape.blocks.forEach(block => bloquesActuales.push(block));
}

function removeBloquesActuales(removeBlock){ //Le pasamos una pieza y guardaremos cada bloque de la pieza en el array bloquesActuales
	posicion = bloquesActuales.indexOf(removeBlock); //Buscamos la posicion del array donde esta ese bloque
	bloquesActuales.splice(posicion, 1); //Con splice borramos el elemento de una posicion concreta
}
...
requestAnimationFrame(pintarBloquesActuales);
...
function TetrisOnlineJ1() {
	this.board = new Board(Tetris.BOARD_WIDTH, Tetris.BOARD_HEIGHT);
}

TetrisOnlineJ1.SHAPES = [I_Shape, J_Shape, L_Shape, O_Shape, S_Shape, T_Shape, Z_Shape];
TetrisOnlineJ1.DIRECTION = {'Left':[-1, 0], 'Right':[1, 0], 'Down':[0, 1]};
TetrisOnlineJ1.BOARD_WIDTH = 10;
TetrisOnlineJ1.BOARD_HEIGHT = 20;
TetrisOnlineJ1.BOARD_COLOR='white';


TetrisOnlineJ1.prototype.init = function(nextShape,currentShape){
	tetris=this;
	/**************
	 EJERCICIO 4
	 ***************/
...
console.log("VUELTA");
	console.log(nextShape);
	 //       this.next_shape = this.create_new_shape(centroNextShape);
       ...
       // this.current_shape = this.create_new_shape(centroS);
var center=Math.trunc(Tetris.BOARD_WIDTH/2);  //el centro del x será la largura del tablero / 2. como puede dar un numero con comas, con esta funcion devolvera el entero.
	var newpoint= new Point(center,0); 

	// Obtener una nueva pieza al azar y asignarla como pieza actual
	//var shapenext=nextShape;
	//var shapeCurrent=currentShape;
	//this.next_shape = new shapenext(newpoint);
	// obtener una nueva pieza al azar y asignarla como pieza actual
	//this.current_shape = new shapeCurrent(newpoint);
if (nextShape=="O_Shape"){
		this.next_shape = new O_Shape(newpoint);
	}else if(nextShape=="S_Shape"){
		this.next_shape = new S_Shape(newpoint);
	}else if(nextShape=="T_Shape"){
		this.next_shape = new T_Shape(newpoint);
	}else if(nextShape=="Z_Shape"){
		this.next_shape = new Z_Shape(newpoint);
	}else if(nextShape=="I_Shape"){
		this.next_shape = new I_Shape(newpoint);
	}else if(nextShape=="J_Shape"){
		this.next_shape = new J_Shape(newpoint);
	}else if(nextShape=="L_Shape"){
		this.next_shape = new L_Shape(newpoint);
	}else{
		console.log("next - No es ninguna");
	}

	if (currentShape=="O_Shape"){
		this.current_shape = new O_Shape(newpoint);
	}else if(currentShape=="S_Shape"){
		this.current_shape = new S_Shape(newpoint);
	}else if(currentShape=="T_Shape"){
		this.current_shape = new T_Shape(newpoint);
	}else if(currentShape=="Z_Shape"){
		this.current_shape = new Z_Shape(newpoint);
	}else if(currentShape=="I_Shape"){
		this.current_shape = new I_Shape(newpoint);
	}else if(currentShape=="J_Shape"){
		this.current_shape = new J_Shape(newpoint);
	}else if(currentShape=="L_Shape"){
		this.current_shape = new L_Shape(newpoint);
	}else{
		console.log("current - No es ninguna");
	}
	// TU CÓDIGO AQUÍ:
	// Pintar la pieza actual en el tablero
	// Aclaración: (Board tiene un método para pintar)
...
TetrisOnlineJ1.prototype.do_move = function(direction) {
...
TetrisOnlineJ1.prototype.nueva_pieza = function(newNextShape) {
	this.board.add_shape(this.current_shape); //Añadimos la pieza actual al tablero;
		var pieza = this.next_shape.constructor;
	newpoint=new Point(centroS,0);
	if (newNextShape=="O_Shape"){
		this.next_shape = new O_Shape(newpoint);
	}else if(newNextShape=="S_Shape"){
		this.next_shape = new S_Shape(newpoint);
	}else if(newNextShape=="T_Shape"){
		this.next_shape = new T_Shape(newpoint);
	}else if(newNextShape=="Z_Shape"){
		this.next_shape = new Z_Shape(newpoint);
	}else if(newNextShape=="I_Shape"){
		this.next_shape = new I_Shape(newpoint);
	}else if(newNextShape=="J_Shape"){
		this.next_shape = new J_Shape(newpoint);
	}else if(newNextShape=="L_Shape"){
		this.next_shape = new L_Shape(newpoint);
	}else{
		console.log("next - No es ninguna");
	}
...
TetrisOnlineJ1.prototype.do_rotate = function(){
...
TetrisOnlineJ1.prototype.animate_shape = function(){
// TU CÓDIGO AQUÍ: genera un timer que mueva hacia abajo la pieza actual cada segundo
	var tetris = this;
	interval=setInterval(function(){tetris.do_move('Down')},1000/nivel);
}
TetrisOnlineJ1.prototype.pausarJuego=function(){
	clearInterval(interval);  //Tenemos que parar el interval porq sino siue intenetando sacar piezas.
	document.getElementById('pause').style.visibility='visible';
	tetris_paused = true;
}
TetrisOnlineJ1.prototype.reanudarJuego=function(){
	document.getElementById('pause').style.visibility = 'hidden';
	tetris_paused = false;
	tetris.animate_shape();
}
