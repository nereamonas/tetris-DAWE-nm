...
socket.emit('nextCurrent-shape',{jugador:"J2",nextShape: this.next_shape.constructor.name, currentShape:this.current_shape.constructor.name});
    // TU CÓDIGO AQUÍ:
...
TetrisJ2.prototype.key_pressed = function(e) {
    ...
    if(!gameOverJ2) {
        if (!tetris_pausedJ2) {  //Si el tetris no está pausado
            if (key==39){  //d - Derecha
            	socket.emit('move',{jugador:"J2",direction: "Right"});
                this.do_move("Right");
            }else if (key==37){  //a - Izquierda
           	socket.emit('move',{jugador:"J2",direction: "Left"});
                this.do_move("Left");
            }else if (key==40){  //s - Abajo
            	socket.emit('move',{jugador:"J2",direction: "Down"});
                this.do_move("Down");
            }else if (key==32){  //x - Barra espaciadora
            	socket.emit('move',{jugador:"J2",direction: "Space"});
                this.do_move("Space");
            }else if (key==38){  //w - Arriba
            	socket.emit('rotate',{jugador:"J2"});
                loadAudio("audio/sonido_rotar.mp3").then( audio => audio.play());
                this.do_rotate();
            }else if (key==80){  //z  - pausar juego
                console.log("Juego pausado");
                this.board.juego_pausado();
                socket.emit('pausarJuego',{jugador:"J2"});
            ...
            if (key == 80) {  //Z  - pausar juego
                console.log("Reanudar juego");
                socket.emit('reanudarJuego',{jugador:"J2"});
...
TetrisJ2.prototype.do_move = function(direction) {
    ...
    if (piezaFinal){
        ...
        socket.emit('nextShape',{jugador:"J2",nextShape: this.next_shape.constructor.name});
...
function TetrisOnlineJ2() {
	this.board = new BoardJ2(Tetris.BOARD_WIDTH, Tetris.BOARD_HEIGHT);
}

TetrisOnlineJ2.SHAPES = [I_Shape, J_Shape, L_Shape, O_Shape, S_Shape, T_Shape, Z_Shape];
TetrisOnlineJ2.DIRECTION = {'Left':[-1, 0], 'Right':[1, 0], 'Down':[0, 1]};
TetrisOnlineJ2.BOARD_WIDTH = 10;
TetrisOnlineJ2.BOARD_HEIGHT = 20;
TetrisOnlineJ2.BOARD_COLOR='white';


TetrisOnlineJ2.prototype.init = function(nextShape,currentShape){
    tetrisJ2=this;
    /**************
     EJERCICIO 4
     ***************/
...
var center=Math.trunc(TetrisJ2.BOARD_WIDTH/2);  //el centro del x será la largura del tablero / 2. como puede dar un numero con comas, con esta funcion devolvera el entero.
	var newpoint= new PointJ2(center,0); 

if (nextShape=="O_ShapeJ2"){
		this.next_shape = new O_ShapeJ2(newpoint);
	}else if(nextShape=="S_ShapeJ2"){
		this.next_shape = new S_ShapeJ2(newpoint);
	}else if(nextShape=="T_ShapeJ2"){
		this.next_shape = new T_ShapeJ2(newpoint);
	}else if(nextShape=="Z_ShapeJ2"){
		this.next_shape = new Z_ShapeJ2(newpoint);
	}else if(nextShape=="I_ShapeJ2"){
		this.next_shape = new I_ShapeJ2(newpoint);
	}else if(nextShape=="J_ShapeJ2"){
		this.next_shape = new J_ShapeJ2(newpoint);
	}else if(nextShape=="L_ShapeJ2"){
		this.next_shape = new L_ShapeJ2(newpoint);
	}else{
		console.log("next - No es ninguna");
	}

	if (currentShape=="O_ShapeJ2"){
		this.current_shape = new O_ShapeJ2(newpoint);
	}else if(currentShape=="S_ShapeJ2"){
		this.current_shape = new S_ShapeJ2(newpoint);
	}else if(currentShape=="T_ShapeJ2"){
		this.current_shape = new T_ShapeJ2(newpoint);
	}else if(currentShape=="Z_ShapeJ2"){
		this.current_shape = new Z_ShapeJ2(newpoint);
	}else if(currentShape=="I_ShapeJ2"){
		this.current_shape = new I_ShapeJ2(newpoint);
	}else if(currentShape=="J_ShapeJ2"){
		this.current_shape = new J_ShapeJ2(newpoint);
	}else if(currentShape=="L_ShapeJ2"){
		this.current_shape = new L_ShapeJ2(newpoint);
	}else{
...
TetrisOnlineJ2.prototype.do_move = function(direction) {
...
TetrisOnlineJ2.prototype.nueva_pieza = function(newNextShape) {
	 this.board.add_shape(this.current_shape); //Añadimos la pieza actual al tablero;
        var pieza = this.next_shape.constructor;
        
	newpoint=new Point(centroS,0);
	if (newNextShape=="O_ShapeJ2"){
		this.next_shape = new O_ShapeJ2(newpoint);
	}else if(newNextShape=="S_ShapeJ2"){
		this.next_shape = new S_ShapeJ2(newpoint);
	}else if(newNextShape=="T_ShapeJ2"){
		this.next_shape = new T_ShapeJ2(newpoint);
	}else if(newNextShape=="Z_ShapeJ2"){
		this.next_shape = new Z_ShapeJ2(newpoint);
	}else if(newNextShape=="I_ShapeJ2"){
		this.next_shape = new I_ShapeJ2(newpoint);
	}else if(newNextShape=="J_ShapeJ2"){
		this.next_shape = new J_ShapeJ2(newpoint);
	}else if(newNextShape=="L_ShapeJ2"){
		this.next_shape = new L_ShapeJ2(newpoint);
	}else{
		console.log("next - No es ninguna");
	}
...
TetrisOnlineJ2.prototype.do_rotate = function(){
...
TetrisOnlineJ2.prototype.animate_shape = function(){
// TU CÓDIGO AQUÍ: genera un timer que mueva hacia abajo la pieza actual cada segundo
    var tetrisJ2 = this;
    intervalJ2=setInterval(function(){tetrisJ2.do_move('Down')},1000/nivelJ2);
}

TetrisOnlineJ2.prototype.pausarJuego=function(){
	tetris_pausedJ2 = true;
        clearInterval(intervalJ2);  //Tenemos que parar el interval porq sino sigue intenetando sacar piezas.
        document.getElementById('pauseJ2').style.visibility='visible';
}

TetrisOnlineJ2.prototype.reanudarJuego=function(){
	document.getElementById('pauseJ2').style.visibility = 'hidden';
        tetris_pausedJ2 = false;
        tetrisJ2.animate_shape();
}
